...
ggplot(melt(wwq_wo_x[1:(ncol(wwq_wo_x)-1)]),aes(x = value)) + 
...
```{r echo=FALSE}
ggplot(wwq,aes(x=factor(quality))) + geom_histogram()
```
...
```{r echo=FALSE}
ggplot(wwq,aes(x=citric)) + 
  geom_histogram(binwidth = 0.02) +
  xlim(0,quantile(wwq$citric,0.99))+
  geom_vline(xintercept = quantile(wwq$citric,0.25),color='red')+
  geom_vline(xintercept = quantile(wwq$citric,0.75),color='red')
```
...
```{r echo=FALSE}
ggplot(wwq,aes(x=sugar)) + 
  geom_histogram(binwidth = 0.04) +
  scale_x_log10(breaks = c(1,2,10,30,50))+
  geom_vline(xintercept = quantile(wwq$sugar,0.25),color='red')+
  geom_vline(xintercept = quantile(wwq$sugar,0.75),color='red')
```
...
ggplot(wwq,aes(x=chlorides)) + 
  ...
  scale_x_log10(breaks=c(0.01,0.05,0.1,0.3))+
  geom_vline(xintercept = quantile(wwq$chlorides,0.25),color='red')+
  geom_vline(xintercept = quantile(wwq$chlorides,0.75),color='red')
```
...
```{r echo=FALSE}
ggplot(wwq,aes(x=freeSO2)) + 
  geom_histogram(binwidth = 2) +
  xlim(0,quantile(wwq$freeSO2,0.999))+
  geom_vline(xintercept = quantile(wwq$freeSO2,0.25),color='red')+
  geom_vline(xintercept = quantile(wwq$freeSO2,0.75),color='red')+
  geom_vline(xintercept = quantile(wwq$freeSO2,0.99),color='blue')
```

More precisely, we removed the top 0.1% and included a blue line that shows the 99% quantile. We can easily see that the top 1% of the free sulfur dioxide values produce a long tail of an otherwise symmetric distribution. The 99% quantile has a value of `r quantile(wwq$freeSO2,0.99)`. The maximum values is located at `r max(wwq$freeSO2)` and therefore an extreme outlier.
...
```{r echo=FALSE}
ggplot(subset(wwq,wwq$density < 1.01),aes(x=density)) + 
  geom_histogram() +
  geom_vline(xintercept = quantile(wwq$density,0.25),color='red')+
  geom_vline(xintercept = quantile(wwq$density,0.75),color='red')
```
...
For our purposes, an outlier is defined as a value falling outside the interval [Q1 - `r a` x IQR , Q3 + `r a` x IQR].
...
There are `r  dim(subset(wwq,wwq$chlorides > q3[5] + a*iq_range[5]))[1]` data points with chloride values `r a` times the interquartile range above the third quartile. Most of them with a quality of 5 and 6. Deleting the outliers would strengthen the correlation between quality and chlorides from `r cor(wwq$quality,wwq$chlorides)` to `r cor(outl$quality,outl$chlorides)`, which would make it interesting to delete the outliers. Nevertheless, as we don't have enough information about the data generation we abstain from deleting that many values.
...
ggplot(data = wwq, aes(x = density, y = alcohol)) + 
  geom_point(alpha = 0.2, position = position_jitter(height = 0.1)) +
...
Although we obtain a correlation coefficient of `r cor(wwq$sugar,wwq$alcohol)` (supporting our assumption), we see a diffuse pattern. Especially for low amounts of residual sugar, there seems to be no or only little influence on the alcohol content. However, in the section on multivariate plots, we will be able to identify interactions between alcohol and sugar when it comes to the wine quality!
...
ggplot(data = wwq, aes(x = density, y = sugar, color = alcohol)) +
  geom_point(alpha = 0.5, position = position_jitter(width=0.0001,height=0.04))+ 
  geom_smooth(method = 'lm', color = 'red') +
...
#ggplot(data = wwq, aes(x=alcohol, y=chlorides)) +
  #geom_point(position = position_jitter(width=0,height=0),alpha = 0.2) +
  #facet_wrap(~quality.grouped)+
  #scale_y_log10()+
  #geom_vline(xintercept = median(wwq$alcohol),color='blue')+ #add median in blue
  #geom_hline(yintercept = median(wwq$chlorides),color='blue') +#add median in blue
  #geom_smooth(method ='lm', 
  #            formula = y~ns(x,2),color = 'red') 
```
...
ggplot(data = wwq, 
  ...
  ggtitle("Chlorides Boxplot for given Qualities") + 
  xlab("Wine Quality (0 worst - 10 best)") + 
